...
class TestGraphRAGQuery(unittest.TestCase):
    def setUp(self):
        ...
        with patch(
            "hugegraph_llm.operators.hugegraph_op.graph_rag_query.PyHugeClient",
                ...
                "objects": [
                    {
                        "label": "person",
                        "id": "person:1",
                        "props": {"name": "Tom Hanks", "age": 67},
                    },
                    {
                        "label": "acted_in",
                        "inV": "movie:1",
                        "outV": "person:1",
                        "props": {"role": "Forrest Gump"},
                    },
                    {
                        "label": "movie",
                        "id": "movie:1",
                        "props": {"title": "Forrest Gump", "year": 1994},
                    },
                ]
            }
        ]
    ...
    @patch(
        "hugegraph_llm.operators.hugegraph_op.graph_rag_query.GraphRAGQuery._gremlin_generate_query"
    )
    def test_run(self, mock_gremlin_generate_query, mock_subgraph_query):
        ...
        mock_gremlin_generate_query.return_value = {
            ...
            "graph_result": [
                "result1",
                "result2",
            ],  # String results as expected by the implementation
        }
    ...
    def test_gremlin_generate_query(self, mock_gremlin_generator_class):
        ...
        mock_gremlin_generator.run.return_value = {
            "result": self.gremlin_query,
            "raw_result": self.gremlin_query,
        }
        self.graph_rag_query._gremlin_generator = mock_gremlin_generator
        self.graph_rag_query._gremlin_generator.gremlin_generate_synthesize.return_value = (
            mock_gremlin_generator
        )
    ...
    @patch(
        "hugegraph_llm.operators.hugegraph_op.graph_rag_query.GraphRAGQuery._format_graph_query_result"
    )
    def test_subgraph_query(self, mock_format_graph_query_result):
        ...
        self.graph_rag_query._extract_labels_from_schema.return_value = (
            ["person", "movie"],
            ["acted_in"],
        )
    ...
    def test_init_client(self):
        ...
        with patch(
            ...
            "hugegraph_llm.operators.hugegraph_op.graph_rag_query.isinstance"
        ) as mock_isinstance:

            # Mock isinstance to avoid type checking issues
            mock_isinstance.return_value = False

            mock_client = MagicMock()
            mock_client_class.return_value = mock_client

            # Create a new instance directly instead of using self.graph_rag_query
            ...
            mock_client_class.assert_called_once_with(
                "127.0.0.1", "8080", "hugegraph", "admin", "xxx", None
            )
    ...
    def test_format_graph_from_vertex(self):
        ...
        original_method = self.graph_rag_query._format_graph_from_vertex
        self.graph_rag_query._format_graph_from_vertex = format_graph_from_vertex
        ...
        try:
            ...
            self.graph_rag_query._format_graph_from_vertex = original_method
    ...
    def test_format_graph_query_result(self):
        ...
        def process_path(path_objects):
            knowledge = "person:1 [label=person, name=Tom Hanks] -[acted_in]-> movie:1 [label=movie, title=Forrest Gump]"
        ...
        original_process_path = self.graph_rag_query._process_path
        original_update_vertex_degree_list = self.graph_rag_query._update_vertex_degree_list
        original_format_graph_query_result = self.graph_rag_query._format_graph_query_result

        self.graph_rag_query._process_path = process_path
        self.graph_rag_query._update_vertex_degree_list = update_vertex_degree_list
        self.graph_rag_query._format_graph_query_result = format_graph_query_result

        try:
            # Run the method
            v_cache, vertex_degree_list, knowledge_with_degree = (
                self.graph_rag_query._format_graph_query_result(query_paths)
            )
            ...
            self.graph_rag_query._process_path = original_process_path
            self.graph_rag_query._update_vertex_degree_list = original_update_vertex_degree_list
            self.graph_rag_query._format_graph_query_result = original_format_graph_query_result
    ...
    def test_extract_labels_from_schema(self):
        ...
        # Temporarily replace the method with our implementation
        original_method = self.graph_rag_query._extract_label_names
        self.graph_rag_query._extract_label_names = mock_extract_label_names
        ...
        try:
            ...
            self.graph_rag_query._extract_label_names = original_method
    ...
    def test_extract_label_names(self):
        ...
        # Temporarily replace the method with our implementation
        original_method = self.graph_rag_query._extract_label_names
        self.graph_rag_query._extract_label_names = extract_label_names
    ...
    def test_get_graph_schema(self):
        ...
        with patch(
            ...
            mock_schema.getRelations.return_value = (
                "[{name: acted_in, sourceLabel: person, targetLabel: movie}]"
            )
