...
class TestCommit2Graph(unittest.TestCase):
    ...
    @patch(
        "hugegraph_llm.operators.hugegraph_op.commit_to_hugegraph.Commit2Graph.init_schema_if_need"
    )
    ...
    @patch(
        "hugegraph_llm.operators.hugegraph_op.commit_to_hugegraph.Commit2Graph._check_property_data_type"
    )
    ...
    def test_handle_graph_creation_success(self):
        ...
        # Call the method
        result = self.commit2graph._handle_graph_creation(
    ...
    @patch(
        "hugegraph_llm.operators.hugegraph_op.commit_to_hugegraph.Commit2Graph._handle_graph_creation"
    )
    ...
    def test_load_into_graph(self, mock_handle_graph_creation, mock_check_property_data_type):
        ...
        vertices = [
            {
                "label": "person",
                "properties": {"name": "Tom Hanks", "age": 67},  # Use integer instead of string
            },
            {
                "label": "movie",
                "properties": {
